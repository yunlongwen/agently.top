...
def _build_hn_prompt(intro, stories_text):
    ...
    return (
        ...
        "2.【核心观点】挑 2-4 条最有料的评论，每条独占一行，格式：\n"
        "   用户 xxx：具体观点或论据（直接引用评论里的关键词）\n"
        "   用户 yyy：具体观点或论据\n"
        "3.【争议/亮点】如果社区有明显分歧、反直觉的见解或技术细节，"
        "用 1-2 句点出关键矛盾点\n"
        "4.【行动启示】这条讨论对后端工程师有什么具体启发（学什么、避什么坑、试什么工具）\n\n"
        "要求：\n"
        "- 每个用户观点必须用换行符(\\n)分隔，绝对不要用分号连在一起\n"
        ...
        "范例：\n"
        '{{"index": 1, "summary": "【SQLite 作者宣布不再接受外部 PR】Hacker News 社区反应两极。\\n'
        '用户 jsmith：单人维护反而保证了代码一致性，Linux 初期也是 Linus 一个人\\n'
        '用户 devguy：2024 年了还拒绝协作是傲慢，Postgres 那套贡献模式才是可持续的\\n'
        '用户 dbexpert：SQLite 测试覆盖率 100% MC/DC，极为罕见，可能正是封闭开发的副产品\\n'
        '【争议】项目所有权与社区贡献如何平衡。\\n'
        '【启示】对核心库维护者：测试覆盖率和一致性比贡献者数量更重要。\\n'
        '【行动】可以查一下自己项目里 SQLite 的使用是否依赖了某个被关闭的 PR。"\n'
        '}}\n\n'
